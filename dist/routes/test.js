...
export async function testRoutes(fastify) {
    fastify.post('/test/insert', {
        ...
        const { dt_firstname, dt_middlename, dt_lastname, dt_session } = request.body;
        try {
            const [result] = await poolAuthentication.execute(`INSERT INTO \`tbl_test\` (\`dt_firstname\`, \`dt_middlename\`, \`dt_lastname\`, \`dt_session\`) VALUES (?, ?, ?, ?)`, [dt_firstname, dt_middlename, dt_lastname, dt_session]);
    ...
    fastify.get('/test/get', {
        ...
        try {
            const [rows] = await poolAuthentication.execute(`SELECT \`dt_no\`, \`dt_firstname\`, \`dt_middlename\`, \`dt_lastname\`, \`dt_session\` FROM \`tbl_test\` ORDER BY \`dt_no\` DESC`);
